	...
	<div class="row">
		<div class="col-lg-9">
				<?php
					...
					{
						$sql = "SELECT * FROM post WHERE id IN (SELECT idPost FROM tag_post_relation WHERE tagName = '$search') ORDER BY id DESC LIMIT $news_per_page OFFSET $skipped_news";
				...
				<?php if($page>0){ ?>
					<a href="reviews.php?page= <?php echo $previous_page ?>">				
						<button class="btn btn-light-blue" name="previous" id="buttons">Cofnij</button>
					</a>
				<?php } ?>	

				<?php if(!isset($end_of_posts)){ ?>
					<a href="reviews.php?page= <?php echo $next_page ?>">
						<button class="btn btn-light-blue" name="next" id="buttons">Dalej</button>
					</a>
				<?php } ?>	
